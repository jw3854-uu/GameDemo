...
{
    private string folderPath = "Assets"; // 默认扫描整个 Assets 文件夹
    ...
    private void OnGUI()
    {
        GUILayout.Label("批量转换 .cs 文件为 UTF-8（带 BOM）", EditorStyles.boldLabel);
        ...
        if (GUILayout.Button("开始转换"))
        {
            if (Directory.Exists(folderPath))
            {
                ...
                EditorUtility.DisplayDialog("完成", "转换完成！（如果没有打印，再试一次）", "确定");
            }
            else
            {
